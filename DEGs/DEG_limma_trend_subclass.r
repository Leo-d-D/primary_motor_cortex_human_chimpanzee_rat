...
run_limmatrend<-function(processed,cellinfo,cov=T,former.meth=''){
  ...
  cellinfo.cov<-cellinfo[,c('subclass','batch')]
  ...
  if(cov){
    design<-model.matrix(formula(paste(c("~ 0 +  subclass", setdiff(colnames(cellinfo.cov),c('subclass'))), collapse = '+')), data=cellinfo.cov)
  }else{
    design <- model.matrix(~ subclass, data=cellinfo.cov)
  }
  subclass_list <- unique(cellinfo.cov$subclass)
  for (i in subclass_list) {
    temp_cellinfo <- cellinfo.cov
    print(i)
    temp_cellinfo$subclass[temp_cellinfo$subclass != i] <- "other"
    temp_cellinfo$subclass%<>%factor()
    temp_cellinfo$batch%<>%factor()
    
    #temp_cellinfo %<>% mutate(subclass = replace(subclass, subclass != i, "other"))
    print(levels(temp_cellinfo$subclass))
    design <- model.matrix(formula(paste(c("~ 0 +  subclass", setdiff(colnames(temp_cellinfo),c('subclass'))), collapse = '+')), data=temp_cellinfo)
    print(design[1,])
    constrasts <- makeContrasts(contrasts=paste(paste0("subclass",i),"-",paste0("subclass","other"),sep=""),levels = design)
